...
4. **PCDN 节点质量参差（2026-09-17 浏览器实测补充）**：mcdn 节点对开放式 Range（`bytes=0-`）返回 200 而非 206，且常在传输数十 KB 后中途断流；部分节点对后续 Range 请求直接重置连接。表现为「同一内容时而能播、时而不能」。每条流除 `baseUrl` 外还有 `backupUrl` 候选，其中含官方镜像（`upos-sz-*.bilivideo.com`、`akamaized.net`），稳定性明显更好。
