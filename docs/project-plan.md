...
音乐库、我的喜欢、自建歌单、最近播放、快照 v3、多轮播放修复、设置页与自动更新（v0.3.0）、托盘与系统媒体控制（v0.3.2）、账号登录与收藏双向同步（v0.4.0）已进入代码；音视频切换及 Linux / 移动端能力仍未实现。下文明确区分当前实现、后续目标和待验收事项；历史实机记录不能替代当前版本验收。
...
窗口最小化时可继续播放。当前 macOS 关闭主窗口会保存并隐藏，后台继续播放，Dock 点击唤回，Cmd+Q / Dock 退出应用；Windows 自 v0.3.2 起关窗进入托盘（托盘菜单为唯一退出路径）；Linux 关窗退出。可配置托盘行为仍是后续能力。
...
| P1 | 系统集成（部分实现） | 媒体快捷键与系统媒体控制已随 v0.3.2 实现；可配置的托盘行为待做 |
| P1 | 其他桌面平台 | 在 macOS / Linux 真机分别构建和验证 |
| P2 | 搜索、登录、收藏夹导入 | 登录与收藏夹导入已随 v0.4.0 实现（扫码 + 双向同步）；搜索仍待确认数据来源 |
...
采用 Tauri 2 + React + TypeScript + Vite，Zustand 管理前端业务状态，Rust 承担内容源访问和回环媒体代理。当前存储为 WebView `localStorage` 快照 v3，兼容 v1 / v2；未接入早期设想的 Tauri Store。视频同步仍为后续模块；系统媒体控制已随 v0.3.2 实现。
...
> 进展（2026-09-24）：账号能力（扫码登录、收藏夹双向同步）已实现并随 v0.4.0 发布，自动更新与设置页已随 v0.3.0 交付，系统媒体控制与托盘已随 v0.3.2 交付；账号能力的调研、语义红线与三轮复审记录见 [login-fav-research.md](login-fav-research.md)。本阶段余下：音视频切换、macOS 后续能力、Linux 适配及移动端评估。

自动更新（Windows 先行，已随 v0.3.0 交付）、音视频切换与账号能力（扫码登录、收藏同步，已随 v0.4.0 交付）在本阶段实现，访问范围按内容权限和登录状态验证。本地歌单、最近播放已提前实现；其余任务包括 macOS 后续能力、Linux 适配及移动端评估。移动端需单独验证后台生命周期、音频会话与 WebView 限制；平台扩展（移动端 / 小程序 / 网页版）的可行性调研已完成（2026-09-22），结论与风险见 [平台扩展可行性评估](platform-feasibility.md)：移动端可行且 Android 直发 APK 优先，小程序与网页版服务器中转不做。失焦场景切歌与红心的入口形态已评估（2026-09-22，[迷你浮窗评估](mini-window-eval.md)）：托盘菜单 + SMTC 媒体会话先行（合计约 2–3.5 天，SMTC 按钮集无 Like、红心落托盘），迷你浮窗按需第二步（存在双写者数据丢失路径，须以「副窗零写路径」为硬约束）。
