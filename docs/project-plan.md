...
当前状态（2026-09-21）：**代码版本 v0.2.0；M0–M2 音频主路径完成，M3 稳定性与交付验收进行中，M4 的本地歌单已提前实现**。当前代码基线为 `853d1ed`，本地 `v0.2.0` 标签已存在。发布资产以 GitHub Releases 为准，本次未重新核验远程发布状态。

音乐库、我的喜欢、自建歌单、快照 v3 和多轮播放修复已进入代码；音视频切换、账号登录、设置页、最近播放、系统媒体控制及 Linux / 移动端能力仍未实现。下文明确区分当前实现、后续目标和待验收事项；历史实机记录不能替代当前版本验收。
...
| localStorage 快照 | 数据版本、库、喜欢、歌单、队列、续播状态、浏览页及上次保存目标 | 是；不含尚未实现的最近播放 |
...
音视频切换与账号能力（扫码登录、收藏同步）在本阶段一起实现，访问范围按内容权限和登录状态验证。本地歌单已提前实现；其余任务包括系统媒体控制、macOS 后续能力、Linux 适配及移动端评估。移动端需单独验证后台生命周期、音频会话与 WebView 限制。
