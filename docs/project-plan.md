...
- [x] 验证至少一次连续播放、切换视频、收起视频、切歌和失败恢复。（视频切换未验证，已按 2026-09-18 调整挪至 M4）
...
音视频切换调整至本阶段，与账号能力（扫码登录、收藏同步）一起实现：视频观看受登录状态与清晰度限制，登录后统一放开。随后做本地歌单和系统媒体控制，再按实际环境验证 macOS / Linux。移动端需要单独验证后台生命周期、系统音频会话与 WebView 限制，必要时引入原生播放实现，复用既有业务接口。

> 进展（2026-09-20）：本地歌单已提前实现——曲目库（全部音乐）、「我的喜欢」固定歌单（红心即加入）、自建歌单增删、解析弹窗选择保存目标均已上线，随快照 v2 持久化，只存曲目引用不占媒体空间。本阶段剩余：音视频切换、账号能力、系统媒体控制、平台扩展验证。
